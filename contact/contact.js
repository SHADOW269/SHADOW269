...
/* ═══════════════════════════════════════
   DISCORD LIVE STATUS — Contact Page
═══════════════════════════════════════ */
(function () {
  ...
  const STATUS_META = {
    online:  { label: 'ONLINE',  color: '#23a55a', glow: 'rgba(35,165,90,.6)',   pulse: true  },
    idle:    { label: 'IDLE',    color: '#f0b232', glow: 'rgba(240,178,50,.6)',  pulse: true  },
    dnd:     { label: 'DND',     color: '#f23f43', glow: 'rgba(242,63,67,.6)',   pulse: false },
    offline: { label: 'OFFLINE', color: '#80848e', glow: 'none',                pulse: false },
  };
  ...
  function applyStatus(status) {
    ...
    const dot   = $('ds-status-dot');
    const label = $('ds-status-label');
    const badge = $('ds-status-badge');
    if (dot) {
      dot.style.background  = m.color;
      dot.style.boxShadow   = m.pulse ? `0 0 8px ${m.glow}` : 'none';
      dot.style.animation   = m.pulse ? 'dsBlip 2s ease infinite' : 'none';
    }
  ...
  function renderActivity(acts) {
    const wrap = $('ds-activity-wrap');
    if (!wrap) return;
    const main   = (acts || []).find(a => a.type !== 4);
    const custom = (acts || []).find(a => a.type === 4);

    if (main) {
      const icon   = ACT_ICON[main.type] || '📌';
      const detail = main.details || '';
      const state  = (main.state && main.state !== detail) ? main.state : '';
      wrap.innerHTML = `
        <div class="ds-act-row">
          <span class="ds-act-icon">${icon}</span>
          <div class="ds-act-body">
            <div class="ds-act-name">${main.name || 'Unknown'}</div>
            ${detail ? `<div class="ds-act-detail">${detail}</div>` : ''}
            ${state  ? `<div class="ds-act-detail" style="opacity:.65">${state}</div>` : ''}
          </div>
        </div>`;
      ...
      wrap.innerHTML = `
        <div class="ds-act-row">
          <span class="ds-act-icon">💬</span>
          <div class="ds-act-body">
            <div class="ds-act-detail" style="color:var(--text2)">${emoji}${custom.state}</div>
          </div>
        </div>`;
    } else {
      wrap.innerHTML = `<div class="ds-act-idle"><span>💤</span> No current activity</div>`;
    }
  }
  ...
  function fallback() {
    applyStatus('offline');
    const wrap = $('ds-activity-wrap');
    if (wrap) wrap.innerHTML = `<div class="ds-act-idle"><span>💤</span> No current activity</div>`;
    const nameEl = $('ds-username');
    if (nameEl) nameEl.textContent = 'SHADOW';
    const handleEl = $('ds-handle');
    if (handleEl) handleEl.textContent = '@shadow269';
    const avatarEl = $('ds-avatar');
  ...
  async function fetchStatus() {
    ...
    try {
      ...
      if (!json.success) throw new Error('Not on Lanyard');
      ...
      if (avatarEl) {
        if (user.avatar) {
          avatarEl.innerHTML = `<img src="https://cdn.discordapp.com/avatars/${USER_ID}/${user.avatar}.png?size=80" alt="avatar" style="width:100%;height:100%;object-fit:cover;border-radius:50%">`;
        } else {
          avatarEl.textContent = (user.global_name || user.username || 'S')[0].toUpperCase();
        }
      }

      const nameEl = $('ds-username');
      if (nameEl) nameEl.textContent = user.global_name || user.username || 'SHADOW';
      const handleEl = $('ds-handle');
  ...
  function startTimer() {
    _timer = setInterval(() => {
      _countdown--;
      updateTimer();
      if (_countdown <= 0) { clearInterval(_timer); fetchStatus(); }
    }, 1000);
  }

  // Expose for manual refresh button
  window.refreshDiscordStatus = fetchStatus;
